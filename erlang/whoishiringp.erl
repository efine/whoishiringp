...
%%%
%%% This Erlang module, whoishiringp, is designed to fetch and display
%%% remote job postings from the "Ask HN: Who is hiring" threads on
%%% Hacker News. The workflow begins with the main/0 function, which
%%% determines the maximum concurrency based on the number of CPUs and
%%% delegates to main/1. The main/1 function initializes necessary
%%% network libraries, retrieves the user profile for "whoishiring", and
%%% processes their submissions. If submissions exist, it proceeds to
%%% print jobs in HTML from the first matching "Who is hiring" post.
%%%
%%% The function get_user/2 fetches a user’s profile and verifies the
%%% user ID to ensure correctness. The print_jobs/3 function locates
%%% the first submission matching the target thread using
%%% get_first_matching_submission/2, extracts its title and child post
%%% IDs ("kids"), and prints the jobs found. The matching logic in
%%% is_matching_submission/1 uses a regular expression to identify
%%% posts with the correct title and type.
%%%
%%% To efficiently process job posts, pget_jobs/3 uses parallel
%%% mapping with a chunk size determined by concurrency limits. The
%%% parallel map implementation (pmap_n/4 and its accumulator variant)
%%% splits the workload into manageable chunks and handles potential
%%% RPC failures by retrying with reduced input. The helper safe_split/2
%%% ensures robust list splitting.
%%%
%%% For each job post, get_job/2 fetches the post data, extracts its
%%% text (ignoring deleted or empty posts), and checks if the text
%%% contains the word "remote" (case-insensitive). Only posts
%%% mentioning "remote" are considered valid remote jobs. The results
%%% are printed as a numbered HTML list.
%%%
%%% Supporting functions include make_url/3 for constructing API URLs,
%%% get_url_json/3 and get_url_json_async/3 for synchronous and
%%% asynchronous HTTP requests, and receive_response/2 for handling
%%% responses with timeouts. The utility to_s/1 converts binaries and
%%% integers to strings for consistent formatting.
%%%
%%% Overall, the module demonstrates robust error handling, efficient
%%% parallel processing, and clear separation of concerns, making it
%%% well-suited for extracting and displaying remote job opportunities
%%% from Hacker News.
%%%--------------------------------------------------------------------
%%% @author Edwin Fine <emofine@gmail.com>
-module(whoishiringp).
...
-spec get_url_json_async(Base, Resource, Id) -> {ok, reference()}
    when Base :: binary(),
         Resource :: binary(),
         Id :: integer() | binary().
...
-spec receive_response(RequestId, Timeout) -> {ok, map()} | {error, timeout}
    when RequestId :: reference(),
    ...
    try
        ...
        _:badrpc ->
            io:format(standard_error,
                      ...
                      [ExtraArgs,
                       [integer_to_list(X) ++ "," || X <- lists:sublist(L, 5)]
                       ++ if length(L) > 5 ->
                                 ["..."];
                             true ->
                                 []
